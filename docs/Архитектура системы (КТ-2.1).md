---
title: Архитектура системы (КТ-2.1)
layout: page
---
...
- `users (1) ── (N) publs` (логическая связь по `users.items`).

![Диаграмма](diagrams/db-schema.svg)
*Источник схемы: [`db-schema.dbml`](diagrams/db-schema.dbml) (рендерить через dbdiagram.io)*

> **Примечание:** Связи `users → actions` и `users → publs` — логические (без FK-ограничений в БД), поддерживаются на уровне приложения.
...
![Registration flow](diagrams/registration.svg)
...
![Auth by login/password](diagrams/auth-login.svg)
...
![Auth via Telegram](diagrams/auth-telegram.svg)
...
![Record filling flow](diagrams/record-filling.svg)
1. Пользователь выбирает публикацию на `/dashboard` (`GET /api/publications/current`).
2. Заполняет форму: география (локализация + координаты), таксономия, параметры сбора материала, количественные характеристики.
...
![Geocoding flow](diagrams/geocoding.svg)
- **Поддержка трёх форматов координат**:
  - **DD** (Decimal Degrees) — десятичные градусы: `55.7558`.
  - **DM** (Degrees + Decimal Minutes) — градусы + десятичные минуты: `55° 45.348'`.
  - **DMS** (Degrees, Minutes, Seconds) — градусы, минуты, секунды: `55° 45' 21.0"`.
- На фронтенде DM и DMS преобразуются в DD через `convertToDM` → `convertToDD`.
- **`GET /api/geo/reverse-geocode`**: бэкенд получает DD через `degrees_n/minutes_n/seconds_n`, конвертирует DMS → десятичные градусы через `geo.dms_to_degrees()`, затем выполняет обратное геокодирование через `geopy.Nominatim.reverse()` (10 запросов/с, JWT).
- **Подсказки регионов**: `GET /api/geo/search?field=region&query=...` — фильтрация `locations.json`.
- **Дополнительно**: проверка вхождения в Уральский регион (`ural_border.geojson`).

**Таксономия**

![Taxonomy flow](diagrams/taxonomy.svg)
- **`GET /api/taxonomy/suggest`** — поиск подсказок по `species_export.csv` (field, query, family, genus).
- **`GET /api/taxonomy/autofill`** — авто-заполнение family/genus по точному совпадению.

**Поддержка**

![Support flow](diagrams/support.svg)
...
## 2. Заключение
Архитектура Faunistica_4.0 разделяет ответственность между веб‑клиентом, API и Telegram‑ботом, что упрощает масштабирование и поддержку. PostgreSQL хранит структурированные данные находок, сервисы геокодирования и таксономии обеспечивают качество и единообразие записей. Логирование действий и валидация на уровне API повышают надежность, а экспорт в табличные форматы делает систему удобной для последующей научной обработки.
