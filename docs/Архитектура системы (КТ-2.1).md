...
> **Примечание:** Связи `users → actions` и `users → publs` отмечены на диаграмме как логические — столбцы `user_id` и `items` не имеют внешнеключевых ограничений в БД и поддерживаются на уровне приложения.
...
<img src="diagrams/record-filling.svg" alt="Record filling flow" style="max-height:300px;"/>
