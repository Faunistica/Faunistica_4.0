...
async def get_user_statistics(session: AsyncSession, user_id: int) -> UserStats:
    ...
    try:
        async with session.begin_nested():
            ...
            combined = union_all(er_family, r_family).subquery()
            result = await session.scalar(
                select(combined.c.family)
                .where(combined.c.family.isnot(None), combined.c.family != "")
                .group_by(combined.c.family)
                .order_by(func.count().desc())
                .limit(1)
            )
        most_common_family = result
    except SQLAlchemyError:
        logger.warning(
    ...
    try:
        async with session.begin_nested():
            ...
            combined = union_all(er_genus, r_genus).subquery()
            result = await session.scalar(
                select(combined.c.genus)
                .where(combined.c.genus.isnot(None), combined.c.genus != "")
                .group_by(combined.c.genus)
                .order_by(func.count().desc())
                .limit(1)
            )
        most_common_genus = result
    except SQLAlchemyError:
        logger.warning(
    ...
    try:
        ...
        total_individuals = (total_individuals or 0) + (result or 0)
    except SQLAlchemyError:
        logger.warning(
    ...
    try:
        ...
        distinct_families = (distinct_families or 0) + (result or 0)
    except SQLAlchemyError:
        logger.warning(
    ...
    try:
        ...
        distinct_genera = (distinct_genera or 0) + (result or 0)
    except SQLAlchemyError:
        logger.warning(
    ...
    try:
        ...
        distinct_species = (distinct_species or 0) + (result or 0)
    except SQLAlchemyError:
        logger.warning(
    ...
    try:
        async with session.begin_nested():
            ...
            combined = union_all(er_year, r_year).subquery()
            result = await session.scalar(
