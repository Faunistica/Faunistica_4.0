...
const PublicationSourceFiles: FC<PublicationSourceFilesProps> = ({ publication }) => {
    const files: SourceFile[] = [
        ...(publication.pdf_file
            ? [{ label: 'PDF', url: publication.pdf_file, icon: FileDown }]
            : []),
        ...(publication.bib_file
            ? [{ label: 'BIB', url: publication.bib_file, icon: FileText }]
            : []),
        ...
        return (
            <SourceButton rightIcon={<ExternalLink className="size-3.5 text-slate-400" />} asChild>
                <a href={file.url} target="_blank" rel="noopener noreferrer">
                    <SourceButtonContent
                        rightIcon={<ExternalLink className="size-3.5 text-slate-400" />}
                    />
                </a>
            </SourceButton>
        );
