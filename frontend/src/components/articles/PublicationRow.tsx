...
export const PublicationRow: FC<PublicationRowProps> = ({ pub, mode }) => {
    return (
        <div
            className={cn(
                'group relative flex flex-col gap-4 rounded-xl border border-border/70 bg-card p-4 shadow-sm transition-all duration-200 hover:border-border/80 hover:shadow-md sm:p-5 lg:flex-row',
                mode === 'suggested' && 'bg-amber-50/30 hover:bg-amber-50/50 dark:bg-amber-950/20 dark:hover:bg-amber-950/30',
            )}
        >
            ...
            <div className="w-full min-w-0 flex-1 space-y-2 pl-1">
                ...
                <div className="flex flex-wrap items-center gap-2">
                    <span className="inline-flex items-center gap-1 rounded-full bg-muted px-2.5 py-0.5 font-mono text-xs font-semibold text-muted-foreground">
                    ...
                    {pub.type && (
                        <Badge
                            variant="outline"
                            className="h-5 rounded-full border-border bg-card px-2.5 text-[10px] font-medium text-foreground"
                        >
                            {pub.type}
                        </Badge>
                    )}
                    {pub.language && (
                        <Badge className="h-5 rounded-full bg-muted px-2.5 text-[10px] font-medium text-foreground">
                            {pub.language}
                        </Badge>
                    )}
                    {pub.ural && (
                ...
                {/* Название публикации */}
                <h4
                    className="line-clamp-2 text-sm/snug font-semibold text-foreground transition-colors group-hover:text-foreground md:text-base"
